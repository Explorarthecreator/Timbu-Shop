...
function Productitem({ product, addToCart, open_modal }) {
  ...
  return (
    <div className="max-w-[258px] border border-[#C6BDDE] p-2 card relative">
      <div className="absolute right-4 top-3">
        {/* <img src={heart} alt=""/> */}
        <svg
          className=" w-5 h-5 hover:cursor-pointer fill-[#190D40] hover:scale-125 hover:fill-[#190D40]"
          viewBox="0 0 20 20"
      ...
      <div className="mt-3 h-full flex flex-col justify-between">
        <div
          className="flex gap-2 text-xs lg:text-sm  hover:cursor-pointer "
          onClick={() => open_modal(product.id)}
        >
          <p className=" w-2/3 md:w-3/5 xl:w-3/4">{product.name}</p>
        ...
        <div className="flex justify-between items-center mt-2">
          <div className=" border-b-2 border-[#121212] w-1/3 md:w-2/5 text-lg md:text-xl xl:text-2xl flex justify-center">
            <span
              className="p-1 cursor-pointer lg:hover:scale-110"
              onClick={() => reduceQuantity(number)}
            >
              -
            </span>
          ...
          <div className="w-3/5 md:w-1/2 text-right">
            <button
              className=" btn btn-sm xl:btn-md p-2 rounded-xl bg-[#190D40] text-white text-xs lg:hover:border-[#190D40] lg:hover:bg-transparent lg:hover:text-[#190D40] lg:hover:scale-105"
